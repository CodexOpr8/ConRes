...
public class UserWindow extends JFrame {
    ...
    private static final Color AMBER  = new Color(180,  90,   0); // used for the countdown warning text
    ...
    private final ExecutorService worker = Executors.newSingleThreadExecutor(r -> {
        Thread t = new Thread(r, "UserWorker"); t.setDaemon(true); return t;
    });
    ...
    private final JLabel    lblTimer = new JLabel(" "); // countdown label — shows time remaining on the held lock
    ...
    private JPanel buildLoginPanel() {
        ...
        h.setFont(new Font("SansSerif", Font.BOLD, 22)); h.setForeground(TXT); card.add(h, g);
        g.gridy = 1; g.gridwidth = 1; card.add(fLabel("Username"), g);
        g.gridx = 1; styleField(fldUser); card.add(fldUser, g);
        g.gridy = 2; g.gridx = 0; card.add(fLabel("Password"), g);
        g.gridx = 1; styleField(fldPass); card.add(fldPass, g);
        g.gridy = 3; g.gridx = 0; g.gridwidth = 2; g.insets = new Insets(18, 4, 4, 4);
        btnLogin.setPreferredSize(new Dimension(260, 40)); card.add(btnLogin, g);
        g.gridy = 4; g.insets = new Insets(4, 4, 4, 4);
        lblMsg.setFont(new Font("SansSerif", Font.PLAIN, 12));
        lblMsg.setHorizontalAlignment(SwingConstants.CENTER); card.add(lblMsg, g);
        ...
        JLabel hint = new JLabel("<html><center><font color='#94a3b8'>"
            ...
            + "Password: [username]123 &nbsp; e.g. karanjot / karanjot123</font></center></html>");
        hint.setFont(new Font("SansSerif", Font.PLAIN, 11));
        hint.setHorizontalAlignment(SwingConstants.CENTER); card.add(hint, g);
    ...
    private JPanel buildSessionPanel() {
        ...
        lblStatus.setFont(new Font("SansSerif", Font.BOLD, 13)); lblStatus.setForeground(TXT);
        ...
        lblTimer.setFont(new Font("Monospaced", Font.BOLD, 11)); lblTimer.setForeground(AMBER);
        toolbar.add(lblTimer, g);
        p.add(toolbar, BorderLayout.NORTH);

        // status bar at the bottom showing the current lock state
        lblBar.setFont(new Font("Monospaced", Font.BOLD, 11)); lblBar.setForeground(SUBTXT);
        lblBar.setBackground(new Color(22, 30, 55)); lblBar.setOpaque(true);
        lblBar.setPreferredSize(new Dimension(0, 26)); lblBar.setBorder(new EmptyBorder(0, 10, 0, 10));
        p.add(lblBar, BorderLayout.SOUTH);

        txtFile.setFont(new Font("Monospaced", Font.PLAIN, 12));
        txtFile.setBackground(new Color(15, 23, 42)); txtFile.setForeground(new Color(190, 240, 200));
        txtFile.setCaretColor(Color.WHITE); txtFile.setBorder(new EmptyBorder(10, 12, 10, 12));
        txtFile.setLineWrap(true); txtFile.setWrapStyleWord(true); txtFile.setEditable(false);
    ...
    private void wire() {
        ...
        // system.login() blocks if 4 users are already active, so it must not run on the edt
        btnLogin.addActionListener(e -> {
            ...
            loggedInUser = user; loggedInId = rec.id;
            ...
            worker.submit(() -> {
                try {
                    ...
                    SwingUtilities.invokeLater(() -> {
                        ...
                        setState(State.IDLE); setBar("  No file open.", SUBTXT);
        ...
        // open for reading — uses trylock, so shows a popup if the lock times out
        btnRead.addActionListener(e -> {
            ...
            worker.submit(() -> {
                try {
                    ...
                    SwingUtilities.invokeLater(() -> {
                        txtFile.setText(content); txtFile.setCaretPosition(0);
                    ...
                    // a writer didn't release in time — notify the user and return to idle
                    SwingUtilities.invokeLater(() -> {
                        setState(State.IDLE); setBar("  Timed out: " + ex.getMessage(), RED);
        ...
        btnClose.addActionListener(e -> {
            stopHoldCountdown();
            setState(State.IDLE); setBar("  Releasing read lock...", SUBTXT);
            worker.submit(() -> {
                system.stopRead(loggedInId);
                SwingUtilities.invokeLater(() -> { txtFile.setText(""); setBar("  File closed.", SUBTXT); });
            });
        });
        ...
        btnWrite.addActionListener(e -> {
            ...
            setBar("  Opening file for writing  (waiting if others are reading)...", SUBTXT);
            worker.submit(() -> {
                try {
                    system.startWrite(loggedInId);
                    String current = system.getSharedFile().getContent();
                    SwingUtilities.invokeLater(() -> {
                        txtFile.setText(current); txtFile.setCaretPosition(0);
                        txtFile.setEditable(true); txtFile.requestFocusInWindow();
                    ...
                    // readers didn't release in time — notify the user and return to idle
                    SwingUtilities.invokeLater(() -> {
                        setState(State.IDLE); setBar("  Timed out: " + ex.getMessage(), RED);
        ...
        btnSave.addActionListener(e -> {
            ...
            stopHoldCountdown(); setState(State.IDLE); txtFile.setEditable(false);
            setBar("  Saving and releasing write lock...", SUBTXT);
            worker.submit(() -> {
                system.commitWrite(loggedInId, newContent);
                SwingUtilities.invokeLater(() -> { txtFile.setText(""); setBar("  File saved.", new Color(74, 222, 128)); });
            });
        });
        ...
        btnCancel.addActionListener(e -> {
            stopHoldCountdown(); setState(State.IDLE); txtFile.setEditable(false);
            setBar("  Cancelling write, releasing lock...", SUBTXT);
            worker.submit(() -> {
                system.cancelWrite(loggedInId);
                SwingUtilities.invokeLater(() -> { txtFile.setText(""); setBar("  Write cancelled.", SUBTXT); });
            });
        });

        // logout — releases the session slot so another waiting user can proceed
        btnLogout.addActionListener(e -> {
            stopHoldCountdown(); setState(State.LOGGED_OUT);
            worker.submit(() -> {
                system.logout(loggedInUser, loggedInId);
                loggedInUser = null; loggedInId = -1;
                SwingUtilities.invokeLater(() -> {
                    setTitle("ConRes — User Portal");
                    fldUser.setText(""); fldPass.setText(""); msg(" ", TXT);
                    btnLogin.setEnabled(true); txtFile.setText(""); txtFile.setEditable(false);
        ...
        // window close — ensures any held locks and session slots are released cleanly
    ...
    // the message is state-aware: tells the user whether to close or save/cancel.
    private void startHoldCountdown(int seconds, State lockState) {
        ...
        holdCountdown.addActionListener(ev -> {
            ...
            if (secondsRemaining > 0) {
                ...
                stopHoldCountdown();
                // fire the correct button — lock is released via the existing handler, not a separate path
    ...
    // helper: creates a right-aligned form label with bold text
    private static JLabel fLabel(String t) {
        JLabel l = new JLabel(t); l.setFont(new Font("SansSerif", Font.BOLD, 12));
        l.setForeground(new Color(15, 23, 42)); l.setPreferredSize(new Dimension(78, 28)); return l;
    }

    // helper: applies consistent styling to a text input field
    private static void styleField(JTextField f) {
        f.setFont(new Font("SansSerif", Font.PLAIN, 13)); f.setPreferredSize(new Dimension(175, 32));
        f.setBorder(BorderFactory.createCompoundBorder(
            new LineBorder(new Color(203, 213, 225), 1, true), new EmptyBorder(4, 8, 4, 8)));
    }

    // helper: creates a styled button with the given label and background colour
    private static JButton btn(String text, Color bg) {
        JButton b = new JButton(text);
        b.setBackground(bg); b.setForeground(Color.WHITE); b.setFont(new Font("SansSerif", Font.BOLD, 12));
